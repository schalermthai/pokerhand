...
describe(RankNames.PAIR, () => {
  ...
  describe('duel', () => {
    ...
    it('pair > high Card', () => {
      expect(pair1.duel(highCard)).toEqual(
        `You win: ${RankNames.PAIR} > ${RankNames.HIGH_CARD}`
      )
      expect(highCard.duel(pair1)).toEqual(
        `You lose: ${RankNames.HIGH_CARD} < ${RankNames.PAIR}`
      )
    })

    it('pair of 2 vs pair of 3: higher pair wins', () => {
      expect(pair2.duel(pair1)).toEqual(`You win: ${RankNames.PAIR} with 3 > 2`)
      expect(pair1.duel(pair2)).toEqual(
        `You lose: ${RankNames.PAIR} with 2 < 3`
      )
    })

    it('pair of 3 vs pair of 3: higher high card wins', () => {
      expect(pair3.duel(pair2)).toEqual(`You win: ${RankNames.PAIR} with 9 > 8`)
      expect(pair2.duel(pair3)).toEqual(
        `You lose: ${RankNames.PAIR} with 8 < 9`
      )
    })
